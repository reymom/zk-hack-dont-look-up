...
impl Protocol {
    ...
    pub fn count_multiplicities(&self, witness: &Witness, statement: &Statement) -> Vec<FieldElement> {
    ...
    fn hash_to_field_single(&self, msg1: &ProverMessage1, index: u8) -> (u64, u64) {
        ...
        (u64::from_le_bytes(hash[0..8].try_into().unwrap()) % self.field_char, u64::from_le_bytes(hash[8..16].try_into().unwrap()) % self.field_char)
    }

    // Hash prover's first message to generate challenge
    fn hash_to_field(&self, msg1: &ProverMessage1) -> FieldElement {
        let ((c0, c1), (c2, c3), (c4, c5)) = (self.hash_to_field_single(msg1, 0),self.hash_to_field_single(msg1, 1),self.hash_to_field_single(msg1, 2) );

        FieldElement::new(vec![c0, c1, c2, c3, c4, c5], self.field_char, self.irr.clone())
    }
    ...
    pub fn prove_round2(&self, r: &FieldElement, msg1: &ProverMessage1, statement: &Statement) -> ProverMessage2 {
